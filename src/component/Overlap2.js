import React from 'react'
import './Overlap2.css';

function Overlap2() {
    return ( 
        
        <section className="Overlap_Section2">
           
            <div class="Overlap_Container2">
                
                <div class="left2"></div>
                    <div class="right2">
                        <div class="Overlap_Content2">
                            <h1>Roadmap</h1>
                            <h2> Stage 1</h2>
                            <p>Create 7777  Cool, exclusive and fun Collectibles Solana Cosmos.</p>
                            <h2> Stage 2 </h2>
                            <p>Achive 5000 Twitter and Discord members</p>
                            <h2> Stage 3 </h2>
                            <p>Anounce Mint Date and mint</p>
                            <h2> Stage 4 </h2>
                            <p>Market integration - DigitalEyes and SolanaArt</p>
                            <p> Will be #1 priority at launch, should happen soon after minting.</p>
                            <h2> Stage 5</h2>
                            <p> Develop community give-back program.</p>
                            <p> An allocation of the royalties will be airdropt to planet holders.</p>

                            {/* <a href="#" class="btn">Click Me</a> */}
                        </div>
                    </div>
            </div>

        </section>
        

        

    /* 
             /*   < /></>!-- *******************   -->
            <!--  !!!!!!!!!!!!!!!!!!!!!!!!!!! -->
            <!--  This Code is for only the floating card in right bottom corner -->
            <!-- ********************  --> 

            <div id="webCifar-sidebox">
            <div id="webCifar">
                <h2 class="logo">Web Cifar</h2>
                <p class="author">Coded By <span>Shaif Arfan</span> </p>
                <div class="items">
                <a href="https://www.youtube.com/channel/UCdxaLo9ALJgXgOUDURRPGiQ" target="_blank" class="item youtubeLink">
                    <svg title="watch how we made this" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                    <path d="M10 12a2 2 0 100-4 2 2 0 000 4z" />
                    <path fill-rule="evenodd" d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.064 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clip-rule="evenodd" />
                    </svg>
                    <p>Watch how we made this.
                    </p>
                </a>
                <a href="https://webcifar.com" target="_blank" class="item">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" />
                    </svg>
                    <p>https://webcifar.com</p>
                </a>

                </div>
                <div class="close">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" />
                </svg>
                </div>
            </div>
            <div id="webCifar-icon">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <p>Info About the pen</p>
            </div>
            </div> */
            
        
    )
}

export default Overlap2
